...
async function callApi(action, data = {}) {
  try {
    const response = await fetch(GAS_URL, { method: "POST", body: JSON.stringify({ action: action, token: API_TOKEN, data: data }) });
    const result = await response.json();
    
    if (action.startsWith('get')) {
      localStorage.setItem('stitchtrack_backup_' + action + (data.orderId ? '_' + data.orderId : ''), JSON.stringify(result));
    }
    return result;
  } catch (err) { 
    ...
    if (action.startsWith('get')) {
      const backup = localStorage.getItem('stitchtrack_backup_' + action + (data.orderId ? '_' + data.orderId : ''));
      if (backup) return JSON.parse(backup);
      if (action === 'getStats') return {activeClients: '--', pendingOrders: '--', fittingReview: '--'};
      return [];
    }
    ...
    setSyncStatus('offline', 'Offline — changes saved locally, will sync when back online');
...
async function syncOfflineQueue() {
  ...
  for (const item of queue) {
    await fetch(GAS_URL, { method: "POST", body: JSON.stringify({ action: item.action, token: API_TOKEN, data: item.data }) });
  }
...
function loadCachedSnapshot() {
  ...
  } catch (err) { /* no cached stats yet — first-ever launch */ }
  ...
  if (document.getElementById('view-customers').classList.contains('active-view')) renderCustomerList();
  if (document.getElementById('view-orders').classList.contains('active-view')) renderOrderList();
}
...
async function initAppData() {
  ...
  try {
    ...
    const [statsRes, customersRes, ordersRes] = await Promise.all([
      fetch(GAS_URL, { method: "POST", body: JSON.stringify({ action: "getStats", token: API_TOKEN, data: {} }) }).then(r => r.json()),
      fetch(GAS_URL, { method: "POST", body: JSON.stringify({ action: "getCustomers", token: API_TOKEN, data: { includeArchived: showArchivedCustomers } }) }).then(r => r.json()),
      fetch(GAS_URL, { method: "POST", body: JSON.stringify({ action: "getOrders", token: API_TOKEN, data: { includeArchived: showArchivedOrders } }) }).then(r => r.json())
    ]);

    localStorage.setItem('stitchtrack_backup_getStats', JSON.stringify(statsRes));
    localStorage.setItem('stitchtrack_backup_getCustomers', JSON.stringify(customersRes));
    localStorage.setItem('stitchtrack_backup_getOrders', JSON.stringify(ordersRes));

    if (statsRes) {
      document.getElementById('s-clients').innerText = statsRes.activeClients ?? '--';
      document.getElementById('s-pending').innerText = statsRes.pendingOrders ?? '--';
      document.getElementById('s-review').innerText = statsRes.fittingReview ?? '--';
    }
...
window.addEventListener('offline', () => setSyncStatus('offline', 'Offline — showing cached data'));
...
function refreshData(p) {
  ...
  const includeArchived = p === 'customers' ? showArchivedCustomers : showArchivedOrders;
...
function renderCustomerList() {
  const listEl = document.getElementById('customer-master-list'); if (!listEl) return;
  let displayData = cache.customers || [];
  if (customerSearchTerm) {
    displayData = displayData.filter(c =>
      String(c.fullName || '').toLowerCase().includes(customerSearchTerm) ||
      String(c.phone || '').toLowerCase().includes(customerSearchTerm)
    );
  }
  if (displayData.length === 0) { listEl.innerHTML = `<p style="font-style:italic; color:var(--muted); text-align:center; padding:20px; font-size: 16px;">No matching clients.</p>`; return; }

  const visibleCount = Math.min(customerPage * PAGE_SIZE, displayData.length);
  const pageData = displayData.slice(0, visibleCount);

  listEl.innerHTML = pageData.map(c => {
  ...
  }).join('') + loadMoreFooter(displayData.length, visibleCount, 'loadMoreCustomers');
}

function renderOrderList() {
  const listEl = document.getElementById('order-master-list'); if (!listEl) return;
  ...
  listEl.innerHTML = pageData.map(o => {
  ...
  }).join('') + loadMoreFooter(displayData.length, visibleCount, 'loadMoreOrders');
}
...
const PHONE_COUNTRIES = {
  NG: { code: '+234', flag: '🇳🇬', name: 'Nigeria',        digits: 11, stripLeadingZero: true,  placeholder: '08031234567' },
  UK: { code: '+44',  flag: '🇬🇧', name: 'United Kingdom', digits: 11, stripLeadingZero: true,  placeholder: '07911123456' },
  US: { code: '+1',   flag: '🇺🇸', name: 'United States',  digits: 10, stripLeadingZero: false, placeholder: '2025551234' }
};
...
function measurementField(id, label, value) {
  ...
  const options = FRACTION_OPTIONS.map(f => `<option value="${f}" ${f === frac ? 'selected' : ''}>${f === '0' ? '0' : f}</option>`).join('');
  return `
    <div>
      <label>${label}</label>
...
function openModal(type, editData = null) {
  ...
  currentModalId = isEdit ? (type === 'customer' ? editData.customerId : editData.orderId) : null;
  currentModalArchived = isEdit ? String(editData.archived).toLowerCase() === 'yes' : false;
  if (archiveBtn) {
    if (isEdit) {
      archiveBtn.style.display = 'block';
      archiveBtn.style.background = currentModalArchived ? 'var(--success)' : 'var(--danger)';
  ...
  if (type === 'customer') {
    ...
    const photoUploadTag = isEdit ? editData.customerId : ("new_" + Date.now().toString(36) + Math.random().toString(36).substr(2, 4));
    title.innerText = isEdit ? "Update Client" : "Register Client";
    let clientProfilePhoto = isEdit ? (editData.photoUrl || '') : ""; 
    const parsedPhone = isEdit ? parsePhoneForEdit(editData.phone) : { country: 'NG', local: '' };
    ...
    const handleAvatarFile = (file) => {
      ...
      const r = new FileReader(); r.onload = async (evt) => {
        ...
        callApi('uploadImage', { base64: comp, name: "avatar_" + photoUploadTag + ".jpg" }).then(res => {
          if(res && res.url) {
    ...
    submit.onclick = () => {
      ...
      if (!fullName || !localDigits) { alert("Full Name and Mobile Number are required."); return; }
      if (!(new RegExp(`^\\d{${cfg.digits}}$`)).test(localDigits)) {
        alert(`${cfg.name} numbers must be exactly ${cfg.digits} digits (e.g. ${cfg.placeholder}).`);
        return;
      }
      if (cfg.stripLeadingZero && localDigits[0] !== '0') {
        alert(`${cfg.name} numbers should start with 0 (e.g. ${cfg.placeholder}).`);
        return;
      }
      const nationalNumber = cfg.stripLeadingZero ? localDigits.slice(1) : localDigits;
      ...
      callApi(isEdit?'updateCustomer':'saveCustomer', payload).then((res) => {
        ...
        if (!isEdit && res.customerId) {
          showToast(`<i class="fas fa-check-circle" style="color:var(--success)"></i> Client #${res.customerId} registered`);
        }
    ...
    const uniqueId = isEdit ? editData.orderId : ("ORD-" + Date.now().toString(36).toUpperCase());
    title.innerText = isEdit ? `Modify Order #${uniqueId}` : `Create Order #${uniqueId}`;
    
    let designPhotosArray = isEdit && editData.designPhotos ? editData.designPhotos.split(',').filter(Boolean) : [];
    let finishedPhotosArray = isEdit && editData.finishedPhotos ? editData.finishedPhotos.split(',').filter(Boolean) : [];
    ...
    document.getElementById('cam_design_camera').onchange = (e) => handleBatchImageUpload(e.target.files, uniqueId, "design", (url) => { designPhotosArray.push(url); renderImageThumbnailsInline('p_design_indicator', designPhotosArray); }, 'p_design_indicator');
    document.getElementById('cam_design_gallery').onchange = (e) => handleBatchImageUpload(e.target.files, uniqueId, "design", (url) => { designPhotosArray.push(url); renderImageThumbnailsInline('p_design_indicator', designPhotosArray); }, 'p_design_indicator');
    document.getElementById('cam_finished_camera').onchange = (e) => handleBatchImageUpload(e.target.files, uniqueId, "finished", (url) => { finishedPhotosArray.push(url); renderImageThumbnailsInline('p_finished_indicator', finishedPhotosArray); }, 'p_finished_indicator');
    document.getElementById('cam_finished_gallery').onchange = (e) => handleBatchImageUpload(e.target.files, uniqueId, "finished", (url) => { finishedPhotosArray.push(url); renderImageThumbnailsInline('p_finished_indicator', finishedPhotosArray); }, 'p_finished_indicator');
    ...
    submit.onclick = () => {
      ...
      if (!desc) { alert("Please enter a design description."); return; }
      if (isNaN(cost) || cost < 0) { alert("Please enter a valid Total Price."); return; }
      const deposit = !isEdit ? (parseFloat(document.getElementById('o_paid').value) || 0) : (Number(editData.amountPaid) || 0);

      submit.disabled = true;
      const payload = { 
        orderId: uniqueId, customerId: custId, 
        designDescription: desc, fabricNotes: document.getElementById('o_fabric').value, 
        totalCost: cost, amountPaid: deposit, dateDue: toSheetDate(document.getElementById('o_due').value), 
...
function showToast(message, duration = 2500) {
  ...
  toast.id = 'app-toast';
  toast.style.cssText = 'position:fixed; bottom:100px; left:50%; transform:translateX(-50%); background:#212529; color:#fff; padding:12px 22px; border-radius:50px; font-weight:800; font-size:14px; z-index:99999; box-shadow:0 8px 20px rgba(0,0,0,0.25); display:flex; align-items:center; gap:8px; white-space:nowrap;';
  toast.innerHTML = message;
  document.body.appendChild(toast);
  setTimeout(() => { if (document.getElementById('app-toast')) toast.remove(); }, duration);
}
...
function toggleArchiveCurrentRecord() {
  ...
  const action = currentModalType === 'customer' ? 'archiveCustomer' : 'archiveOrder';
  const payload = currentModalType === 'customer'
    ? { customerId: currentModalId, archived: willArchive }
    : { orderId: currentModalId, archived: willArchive };

  callApi(action, payload).then(res => {
    if (res && res.status !== "queued" && res.success === false) {
      alert("Could not update archive status: " + (res.error || "Unknown error"));
      return;
    }
    closeModal();
...
function handleBatchImageUpload(filesList, trackingId, prefix, cb, indicatorId) {
  ...
  if (indicatorEl && indicatorEl.parentElement) {
    ...
    progressEl.style.cssText = 'width:100%; font-size:12px !important; font-weight:800 !important; color:var(--primary); text-align:center; padding:6px 0;';
  ...
  files.forEach((file, i) => {
    ...
    r.onload = async (e) => {
      ...
      callApi('uploadImage', { base64: comp, name: `${prefix}_${trackingId}_${i}_${Date.now()}.jpg` }).then(res => {
...
async function compileStudioInvoice() {
  ...
  const designThumbs = orderItem.designPhotos ? orderItem.designPhotos.split(',').filter(Boolean).map(u => `<img src="${getDirectImageUrl(u)}" class="report-thumb">`).join('') : '';

  const payments = await callApi('getPayments', { orderId }) || [];
  const paymentsRows = payments.length === 0
    ? `<tr><td colspan="3" style="text-align:center; color:#666;">No payments recorded</td></tr>`
    : payments.map(p => `<tr><td>${p.date||''}</td><td>${p.method||'Cash'}</td><td>₦${Number(p.amount).toLocaleString()}</td></tr>`).join('');
...
(function initIosInstallBanner() {
  if (isIosDevice() && !isRunningStandalone() && !localStorage.getItem('stitchtrack_ios_banner_dismissed')) {
    const b = document.getElementById('iosInstallBanner');

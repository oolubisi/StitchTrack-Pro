...
function openModal(type, editData = null) {
  ...
  if (type === 'customer') {
    ...
    body.innerHTML = `
      ...
      <label>Mobile Number <span style="color:var(--danger)">*</span></label><input id="c_phone" type="tel" inputmode="numeric" maxlength="11" placeholder="08031234567" value="${isEdit?editData.phone:''}" oninput="this.value=this.value.replace(/\D/g,'').slice(0,11)">
    ...
    submit.onclick = () => {
      ...
      if (!fullName || !phone) { alert("Full Name and Mobile Number are required."); return; }
      if (!/^\d{11}$/.test(phone)) { alert("Mobile Number must be exactly 11 digits (e.g. 08031234567)."); return; }
